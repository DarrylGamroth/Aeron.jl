...
function Base.show(io::IO, mime::MIME"text/plain", h::Header)
    ...
    println(io, "  frame length: ", frame.frame_length)
    println(io, "  version: ", frame.version)
    println(io, "  flags: ", frame.flags)
    println(io, "  type: ", frame.type)
    println(io, "  term offset: ", frame.term_offset)
    println(io, "  session id: ", frame.session_id)
    println(io, "  stream id: ", frame.stream_id)
    println(io, "  term id: ", frame.term_id)
end
